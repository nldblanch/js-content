...
describe("Race Day", () => {
  ...
  test("early adult gets +1000 race number and 9:30 am", () => {
    ...
    expect(message).toMatch(
      `Your race number is ${raceNumber}. You will race at ${startTime}.`
    );
  });
  ...
  test("youth (under 18) runs at 12:30 pm regardless of registration", () => {
    ...
    expect(a.message).toMatch(
      `Your race number is ${a.raceNumber}. You will race at ${a.startTime}.`
    );
    ...
    expect(b.message).toMatch(
      `Your race number is ${b.raceNumber}. You will race at ${b.startTime}.`
    );
  });
    ...
    combos.forEach((c) => {
      ...
      if (c.msg) {
        expect(info.message).toMatch(
          new RegExp(
            `Your race number is ${info.raceNumber}\\. You will race at ${c.msg}\\.`
          )
        );
      } else {
