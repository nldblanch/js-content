...
const RepoIndex: React.FC<RepoIndexProps> = ({ onSelectRepo, onNewRepo }) => {
  ...
  if (loading) {
    return (
      <div className="bg-[#0d1117] min-h-full flex items-center justify-center h-64 text-[#8b949e] animate-pulse">
            ...
            <div key={path} className="py-6 flex justify-between items-start transition-colors duration-150">
              <div className="flex-1">
                ...
                <p className="text-[14px] text-[#8b949e] max-w-2xl mt-1 mb-3">
                  Source files located at <span className="font-mono text-xs bg-[#161b22] px-1.5 py-0.5 rounded border border-[#30363d]">{path}</span>
                </p>
