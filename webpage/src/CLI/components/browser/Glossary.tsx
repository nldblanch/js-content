...
export default function Glossary() {
  ...
  const renderCommandCard = (
    { command, description, flags, args }: CommandDef,
    type: "shell" | "git",
  ) => {
    ...
    return (
      <div
        key={command}
        className={`bg-white p-5 rounded-lg border border-slate-200 shadow-sm hover:shadow-md hover:border-${themeColor}-400 transition-all flex flex-col h-full`}
      >
        <div className="mb-2">
          <code
            className={`text-${themeColor}-700 bg-${themeColor}-50 px-2 py-1 rounded font-mono text-sm font-semibold`}
          >
            {command}
          </code>
        </div>
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {args.map((arg) => (
                    ...
                    >
                      <code className="text-slate-800 font-mono font-medium whitespace-nowrap">
                        {arg.name}
                      </code>
