...
export const Heading = ({
  ...
  const headingNode = (() => {
    if (children) {
      return <Tag className={twMerge(baseClass, variantStyles[variant], fontStyles[font], className)}>{children}</Tag>;
    }
    ...
    if (!last) {
      return <Tag className={twMerge(baseClass, variantStyles[variant], fontStyles[font], className)}>{safeText}</Tag>;
    }

    return (
      <Tag className={twMerge(baseClass, variantStyles[variant], fontStyles[font], className)}>
        {words.join(' ')} {highlight ? <HighlightedWord word={last} underline={underline} /> : last}
      </Tag>
    );
  })();
