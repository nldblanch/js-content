...
const checklist = [
  {
    title: 'Software:',
    items: ['Node.js (package manager)', 'Visual Studio Code (code editor)', 'Git (version control system)'],
  },
  {
    title: 'Tools',
    items: [
    ...
    content: [
      {
        ...
        moreInfo:
          'In Company Portal, install the approved Node.js package. After it installs, open a Command Terminal (NOT a Powershell) and run: node -v and npm -v to check that it has installed. If you get an error or it says the commands aren’t recognized, try restarting your terminal.',
        instructions: [
      ...
      },
      {
        ...
        moreInfo:
          "After installing, open VS Code and use File → Open Folder… to open the project. You won't have the project yet if you haven't completed the next step (cloning the repo).",
        instructions: [
          '- Go to the Company Portal app on your computer.',
      ...
      },
      {
        ...
        moreInfo:
          'In the GitHub repository, click the green Code button and copy the URL. In your Command Terminal, navigate to the folder where you want to put the project (e.g. Documents) and run: git clone [paste URL here]. This will create a new folder with the project files.',
        instructions: [
    ...
    content: [
      {
        ...
        moreInfo:
          'Try restarting your terminal or computer first. If that doesn’t work, you may need to add Node.js to your system PATH manually. You can search online for instructions on how to do this for your operating system.',
      },
      {
        title: 'Jest not found',
        info: 'Have you run npm install?',
        moreInfo:
          'Make sure you have changed directory into the same level that the package.json file is in before running npm install. If you have and it’s still not working, try deleting node_modules and package-lock.json and running npm install again.',
      },
      {
        title: "Can't find script",
        info: 'Make sure you are running the command in the correct directory',
        moreInfo:
          'Some commands need to be run in specific subdirectories. Check the instructions for the command you’re trying to run and make sure you’ve changed into the correct directory using cd [directory name].',
      },
    ],
  },
  commands: {
...
export default function GettingStarted() {
  return (
    <div className="page-container">
      ...
      <div id={sections.installation.id} className="mt-16 justify-items-center space-y-6 w-full">
        ...
        {sections.installation.content.map(({ title, info, moreInfo, instructions }, idx) => {
          return (
            <section
              key={title}
              className="bg-black-800 rounded-lg w-full text-white justify-center items-center flex flex-col py-4"
            >
              <Heading
                as="h2"
                noWrapper
      ...
      {/* Common Errors */}
      <div
        id={sections.commonErrors.id}
        className="mt-16 p-6 space-y-6 border-orange border-2 justify-items-center w-full bg-black-800 rounded-4xl"
      >
