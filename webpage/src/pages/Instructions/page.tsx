...
export default function InstructionsPage() {
  ...
  if (docs.length === 0) {
    return (
      <div className="page-container">
        <h1 className="text-4xl font-libre font-extrabold text-white">Instructions</h1>
        <p className="text-gray font-fira text-2xl">
          No docs found. Check the glob path in
          <code className="text-blue bg-black-900 px-2 py-0.5 rounded">useChallengeInstructions</code>.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid h-screen overflow-hidden" style={{ gridTemplateColumns: '280px 1fr' }}>
      ...
      <section aria-label="Instructions" className="overflow-y-auto h-full bg-black-800 px-10 py-8">
        {!slug && <MarkdownRenderer markdown={repoReadme} />}

        {slug && !current && (
          <div className="page-container">
            <h1 className="text-4xl font-libre font-extrabold text-white">Instructions</h1>
            <p className="text-gray font-fira text-2xl">Unknown challenge. Pick one from the list.</p>
          </div>
        )}
        ...
        {current && (
          <>
            <header className="flex items-center justify-between gap-3 mb-8">
              ...
              <h1 className="text-4xl font-libre font-extrabold text-white text-center flex-1 px-4">{current.title}</h1>
