...
describe("FileAccordion", () => {
  ...
  test("renders title link and is collapsed by default", () => {
    ...
    expect(
      screen.getByRole("link", { name: "whale talk" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: "function documentation" }),
    ).toBeNull();
  });
  ...
  test("opens and closes when clicking the toggle button", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "function documentation" }),
    ).toBeInTheDocument();
  ...
  test("builds file links with slugified hash anchors", async () => {
    ...
    expect(link).toHaveAttribute(
      ...
      expect.stringContaining(
        "/instructions/8-whale-talk#function-documentation",
      ),
    );
  });
