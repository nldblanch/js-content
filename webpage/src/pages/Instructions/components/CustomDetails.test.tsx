...
describe('CustomDetails Component', () => {
  ...
  it('opens the modal when clicking a standard summary', async () => {
    ...
    expect(screen.queryByText('This is the hidden answer')).not.toBeInTheDocument();
  });

  it("reveals content immediately if summary contains 'table of contents'", async () => {
    const user = userEvent.setup();
  ...
  it("reveals content after clicking 'Show Content' in the modal", async () => {
    renderDetails('Example Summary', 'Example Content');

    // Click summary to open modal
    const user = userEvent.setup();
    await user.click(screen.getByText('Example Summary'));
  ...
  it('toggles content off when clicked while already expanded', async () => {
    const user = userEvent.setup();
