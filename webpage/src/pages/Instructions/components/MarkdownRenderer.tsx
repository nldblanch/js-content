...
export function MarkdownRenderer({ markdown, stripFirstH1, headingIdPrefix }: Props) {
  const normalized = useMemo(() => (stripFirstH1 ? stripLeadingH1(markdown) : markdown), [markdown, stripFirstH1]);
    ...
    <div className="markdown max-w-3xl">
      <ReactMarkdown
        ...
        components={{
          ...
          p: ({ children, ...props }) => (
            <p {...props} className="text-gray font-fira text-xl leading-relaxed mb-4">
              {children}
            </p>
          ),
          pre: ({ children, ...props }) => (
          ...
          code: ({ children, ...props }) => (
            <code {...props} className="bg-black-900 text-blue font-fira px-1.5 py-0.5 rounded text-lg">
              {children}
            </code>
          ),
          ...
          blockquote: ({ children, ...props }) => (
            <blockquote {...props} className="border-l-4 border-blue pl-4 my-4 text-gray font-fira text-xl italic">
              {children}
            </blockquote>
          ),
          details: ({ children }) => <CustomDetails>{children}</CustomDetails>,
          ul: ({ children, ...props }) => (
            <ul {...props} className="list-disc list-inside text-gray font-fira text-xl mb-4 space-y-2">
              {children}
            </ul>
          ),
          ol: ({ children, ...props }) => (
            <ol {...props} className="list-decimal list-inside text-gray font-fira text-xl mb-4 space-y-2">
              {children}
            </ol>
          ),
          li: ({ children, ...props }) => (
          ...
          hr: ({ ...props }) => <hr {...props} className="border-gray/20 my-8" />,
          ...
          table: ({ children, ...props }) => (
            <div className="overflow-x-auto mb-4">
              <table {...props} className="w-full font-fira text-xl border-collapse">
                {children}
              </table>
            </div>
          ),
          ...
          td: ({ children, ...props }) => (
            <td {...props} className="text-gray px-4 py-2 border-b border-gray/10">
              {children}
            </td>
          ),
        }}
